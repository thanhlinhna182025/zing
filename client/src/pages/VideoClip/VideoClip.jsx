...
const VideoClip = () => {
    ...
    dispatch(getVideo(id))
      ...
      .then((result) => {
        ...
        if (result?.streaming?.mp4['720p']) {
          videoRef.current.src = result?.streaming?.mp4['720p']
        } else if (result?.streaming?.mp4['480p']) {
          videoRef.current.src = result?.streaming?.mp4['480p']
        } else {
          videoRef.current.src = result?.streaming?.mp4['360p']
        }
        videoRef.current.poster = result?.thumbnailM
      })
  ...
  return (
    ...
    >
      ...
      <div className='group/video flex w-full flex-col items-start justify-center lg:flex-row' ref={videoContainerRef}>
        ...
        <div className={`${hasFullScreen ? 'hidden' : `${ColorBg400} flex w-full flex-col p-4 lg:ml-5 lg:w-[30%] `} `}>
